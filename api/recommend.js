export default async function handler(req, res) {
  ...
  try {
    ...
    const excludeInstructions = exclude && exclude.length > 0 ? '\nŞu eserleri kesinlikle listeye dahil etme, bunlar zaten önerildi: [' + exclude.map(t => '"' + t.replace(/"/g, '') + '"').join(', ') + ']' : '';

    const countRule = exclude && exclude.length > 0 ? 'kesinlikle 5 adet içerik' : 'en az 5, en fazla 10 adet içerik';

    const systemPrompt = `Sen edebi ve kültürel kürasyon yapan uzman bir stratejistsin. Kullanıcının verdiği filme, diziye, oyuna veya müziğe bakarak sadece "tür" olarak değil; "Ana Tema (Core Tropes)" ve "Üslup/Atmosfer (Vibe & Tone)" açısından benzer hisler uyandıran ${countRule} önermelisin.
